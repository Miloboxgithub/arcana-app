...
const PRESET_AVATARS_DATA = [
  { id: 'joker', label: 'JOKER', node: <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',height:'100%'}}><rect width="100" height="100" fill="#0a0a0a"/><polygon points="0,0 22,0 0,22" fill="#C3002F" opacity="0.8"/><polygon points="100,100 78,100 100,78" fill="#C3002F" opacity="0.4"/><path d="M20 100 Q25 72 50 68 Q75 72 80 100 Z" fill="#1a1a2e"/><path d="M36 100 Q41 76 50 73 Q59 76 64 100 Z" fill="#f0f0f0" opacity="0.9"/><path d="M48 74 L52 74 L54 100 L46 100 Z" fill="#C3002F"/><ellipse cx="50" cy="44" rx="18" ry="20" fill="#f5dcc8"/><ellipse cx="50" cy="30" rx="19" ry="14" fill="#111"/><path d="M68 30 Q74 26 70 38 Q66 34 64 40" fill="#111"/><path d="M32 30 Q28 26 31 40 Q34 36 36 42" fill="#111"/><path d="M33 34 Q42 26 50 30 Q58 26 67 34" fill="#111"/><rect x="35" y="44" width="11" height="7" rx="3" fill="none" stroke="#333" strokeWidth="1.2"/><rect x="54" y="44" width="11" height="7" rx="3" fill="none" stroke="#333" strokeWidth="1.2"/><line x1="46" y1="47" x2="54" y2="47" stroke="#333" strokeWidth="1.2"/><ellipse cx="40" cy="48" rx="3" ry="3.5" fill="#1a1a1a"/><ellipse cx="59" cy="48" rx="3" ry="3.5" fill="#1a1a1a"/><circle cx="41" cy="47" r="0.8" fill="white" opacity="0.8"/><circle cx="60" cy="47" r="0.8" fill="white" opacity="0.8"/><rect x="0" y="95" width="100" height="2" fill="#C3002F" opacity="0.6"/></svg> },
  { id: 'ryuji', label: 'SKULL', node: <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',height:'100%'}}><rect width="100" height="100" fill="#0a0a0a"/><polygon points="0,0 22,0 0,22" fill="#E8C840" opacity="0.8"/><path d="M22 100 Q26 72 50 68 Q74 72 78 100 Z" fill="#1a1220"/><ellipse cx="50" cy="44" rx="18" ry="20" fill="#f0c880"/><ellipse cx="50" cy="29" rx="19" ry="13" fill="#E8C840"/><ellipse cx="40" cy="48" rx="4" ry="4" fill="#1a1a1a"/><ellipse cx="60" cy="48" rx="4" ry="4" fill="#1a1a1a"/><circle cx="42" cy="46" r="1.2" fill="white" opacity="0.9"/><circle cx="62" cy="46" r="1.2" fill="white" opacity="0.9"/><rect x="0" y="95" width="100" height="2" fill="#E8C840" opacity="0.6"/></svg> },
]

function getAvatarNode(avatarId: string): React.ReactNode {
  if (avatarId.startsWith('data:')) {
    return <img src={avatarId} alt="avatar" style={{width:'100%',height:'100%',objectFit:'cover'}} />
  }
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-start', gap: 10 }}>
      ...
      <div style={{
        ...
        {avatarNode ? (
          <div style={{ width: 32, height: 32, overflow: 'hidden' }}>{avatarNode}</div>
        ) : (
          <span style={{ fontFamily: 'Share Tech Mono,monospace', fontSize: 8, color: 'var(--gold)', letterSpacing: 0.5 }}>
...
function SystemBubble({ text }: { text: string }) {
  return (
    <div style={{ textAlign: 'center', padding: '6px 0' }}>
      <span style={{ fontFamily: 'Share Tech Mono,monospace', fontSize: 8, letterSpacing: 2, color: 'var(--muted)', background: 'var(--card2)', padding: '3px 12px', borderRadius: 2, display: 'inline-block' }}>
        {text}
      </span>
    </div>
  )
...
export default function MorganaPage({ onClose }: MorganaPageProps) {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      const ctx = buildCtx()
      const history: ChatMessage[] = messages.slice(1).map(m => ({ role: m.role as 'user' | 'assistant', content: m.content }))
